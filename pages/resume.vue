<template>
  <div class="relative w-full min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 text-slate-900">
    <!-- Resume Container -->
    <div id="resume-content"
      class="max-w-4xl mx-auto p-8 bg-white shadow-lg print:shadow-none print:p-0 print:bg-white">
      <!-- Header Section -->
      ...
      <section class="mb-8">
        <h2 class="text-2xl font-bold text-slate-900 mb-4 flex items-center gap-2">
          <BriefcaseBusiness :size="24" class="text-indigo-600" />
          Experience
        </h2>
        <div class="space-y-6">
          <article v-for="(job, index) in resumeData.experience" :key="index" class="border-l-4 border-indigo-300 pl-4">
      ...
      <section class="mb-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div v-for="skillGroup in resumeData.skills" :key="skillGroup.category"
            class="border-l-4 border-indigo-300 pl-4">
            <h3 class="font-bold text-slate-900 mb-3">{{ skillGroup.category }}</h3>
            <div class="flex flex-wrap gap-2">
              <span v-for="skill in skillGroup.skills" :key="skill"
    ...
    <button @click="downloadPDF"
      ...
      :class="{ 'opacity-50 cursor-not-allowed': isDownloading }" :disabled="isDownloading" title="Download as PDF">
